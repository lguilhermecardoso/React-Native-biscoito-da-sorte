...
function App () {
  ...
  const [image, setImage] = React.useState(require('./src/biscoito.png'));
  ...
  function openCookie() {
    const rndInt = Math.floor(Math.random() * phases.length)
    setPhrase(`"${phases[rndInt]}"`);
  ...
  return (
    <View style={styles.container}>
      <Image
        ...
        style={styles.img}
      />
      <Text style={styles.textPhase}>{phrase}</Text>
      <TouchableOpacity style={styles.button} onPress={ openCookie }>
        <View style={styles.btnArea}>
          <Text style={styles.btnText}>
            {!phrase ? 'Quebrar biscoito' : 'Abrir novo biscoito'}
